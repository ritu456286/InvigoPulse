  ...
  const filterResults = (event) => {
    ...
    const filteredData = productSales.filter((item) => {
      ...
      const inventoryIdMatch =
        item.InventoryID &&
        item.InventoryID.toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      const companynameMatch =
        item.companyname &&
        item.companyname.toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      const SizeMatch =
        item.Size &&
        item.Size.toString().toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      const brandMatch =
        item.Brand &&
        item.Brand.toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      const descriptionMatch =
        item.Description &&
        item.Description.toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      const PriceMatch =
        item.Price &&
        item.Price.toString().toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      const CityMatch =
        item.City &&
        item.City.toString().toLowerCase().includes(searchTerm.toLowerCase()); // Check if not null
      // Check if not null
  ...
  return (
    <div className="py-10 text-center">
      ...
      <form onSubmit={filterResults} className="m-6">
        ...
        <button
          type="submit"
          className="bg-red-700 hover:bg-red-600 text-white font-bold py-2 px-4 rounded ml-2 mt-3"
        >
          Submit
        </button>
      </form>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <button
          onClick={() => paginate(currentPage + 1)}
          disabled={indexOfLastItem >= productSales.length}
        >
          Next
        </button>
      </div>
